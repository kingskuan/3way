...
一个跑在自己电脑或 Railway 上的加密货币（含股票 perps）**永续合约网格交易机器人**，同时支持十家交易所：**Decibel**（Aptos）、**Extended**（Starknet）、**RISEx**、**Ondo Perps**（股票 perps）、**perpl.xyz**（Monad L1）、**StandX**（BSC）、**Bitget**、**Bitunix**、**Phoenix**（Solana）、**Nado**（Ink Chain）。每所可同时各跑一个网格策略，统一在浏览器仪表盘里监控、AI Autopilot 全权托管，Telegram/Webhook 推送。
...
| 功能 | 说明 |
|---|---|
| 多所并行 | Decibel / Extended / RISEx / Ondo / Perpl / StandX / Bitget / Bitunix / Phoenix / Nado，各自独立运行一个网格机器人，互不影响 |
...
- 📊 **总览页**：全部已接入交易所的余额、权益、总盈亏、已实现/未实现盈亏、收益率、成交量、完成格数实时刷新（SSE 秒级推送）
...
4. **小资金起步**：首次实盘用你亏得起的钱。
5. **本地运行时默认只监听本机**（localhost）。若日志提示监听 `0.0.0.0`，说明局域网内其他设备也能访问，请确保网络环境可信。
6. 本程序不依赖官方远程服务器，不会把密钥/数据上传给作者或第三方；但如果你自己把它部署到 Railway 等云平台并生成了公网 URL，那个 URL 本身就相当于"远程服务器"——任何拿到网址 + 仪表盘密码的人，都能操作里面配置的真实资金，请只把网址/密码给完全信任的人，绝不要发到群里或公开（详见下一节）。

---

## 十七、多人使用 / 开放给朋友

⚠️ 本项目目前是**单租户**设计：一份部署 = 一份 `.env` = 一个仪表盘密码，能登录仪表盘的人就能操作里面配置的**全部**交易所真实资金（开仓、调杠杆、平仓），没有"只读"或"分权限"账号。给朋友使用前请先想清楚要哪种方式。

### 方式一：让朋友 fork 仓库，自己独立部署（推荐）

每个人 fork 这个仓库到自己的 GitHub 账号，按第三·五节自己开一个 Railway 项目（或本地跑），填**自己的**交易所 API 密钥/私钥、自己设一个仪表盘密码。这样每个人的资金、密钥、风险完全独立，互不影响，也是本项目原本的设计方式。步骤简述：先打开这个仓库页面右上角的 **Fork** 按钮，fork 到朋友自己的 GitHub 账号下；朋友再按自己仓库里的第三·五节流程部署到自己的 Railway（或本地）；最后朋友填自己的交易所密钥和自己的 `DASHBOARD_PASSWORD`，和你的部署完全隔离。

### 方式二：只是想让朋友"看着玩"，不动真钱

给朋友一份新部署，全部交易所保持默认的 `paper`（模拟）模式，不填任何真实密钥。这种情况下随便分享网址都没有资金风险，因为没有连真实账户。

### 方式三：共用你现在这一套账号

目前**不支持**给不同人分配不同权限（比如只能看、不能操作）。如果需要这个功能，需要额外开发"只读账号 / 分权限"能力；在没有这个功能之前，请不要把现有正在跑真钱的部署网址和密码分享给别人。
