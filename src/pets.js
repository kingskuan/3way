...
// 换算：feed 单位 = USDC 累积交易量（跟 bot.stats.volume 一致）。
export const LEVEL_THRESHOLDS = [
  0,             // L1 灵卵（起点）
  500_000,       // L2 幼
  100_000_000,   // L3 少（1 亿）
  200_000_000,   // L4 成（2 亿）
  500_000_000,   // L5 强（5 亿）
  1_000_000_000, // L6 神（10 亿）
  5_000_000_000, // L7 圣（50 亿）
];

// 每所专属宠物：中国古代神兽 · 7 阶进化 · gpt-image-1 手绘立绘
//   stageNames: 每阶专名（UI 显示）
//   fallback: 图片加载失败时的 emoji 兜底
//   image path 惯例：/pets/{key}-lv{1..7}.png（lv7 立绘待 gen，暂用 fallback emoji）
export const PET_SPECIES = {
  de: {
    name: '朱雀', theme: '南方火神 · 声鸣震天',
    stageNames: ['灵卵', '雏雀', '鸣禽', '赤翎', '焚天雀', '朱雀神', '焚天圣王'],
    fallback: ['🥚', '🐣', '🐦', '🦜', '🔥', '🦅', '🌅'],
    color: '#dc2626',
  },
  ex: {
    name: '青龙', theme: '东方木神 · 千里游延',
    stageNames: ['龙卵', '虺', '蛟', '应龙', '蟠龙', '青龙神', '苍穹圣龙'],
    fallback: ['🥚', '🐛', '🐍', '🦎', '🐊', '🐉', '🐲'],
    color: '#0ea5e9',
  },
  rs: {
    name: '鲲鹏', theme: '化鲲为鹏 · 扶摇直上',
    stageNames: ['鱼卵', '幼鲲', '大鲲', '巨鲲', '化鹏', '鲲鹏神', '天穹鲲鹏'],
    fallback: ['🥚', '🐟', '🐠', '🦈', '🐋', '🦅', '🌌'],
    color: '#10b981',
  },
  on: {
    name: '玄武', theme: '北方水神 · 龟蛇合体',
    stageNames: ['神卵', '灵龟', '玄龟', '蛇龟合', '千岁玄', '玄武神', '万古玄武'],
    fallback: ['🥚', '🐚', '🐢', '🐍', '🌊', '🐉', '⚓'],
    color: '#0284c7',
  },
  pl: {
    name: '白虎', theme: '西方金神 · 永镇长存',
    stageNames: ['虎卵', '幼虎', '猛虎', '金虎', '山君', '白虎神', '破天白虎'],
    fallback: ['🥚', '🐈', '🐅', '🐆', '🐯', '👑', '⚔️'],
    color: '#e2e8f0',
  },
  sx: {
    name: '麒麟', theme: '仁兽至圣 · 现世太平',
    stageNames: ['麟卵', '幼麟', '花麟', '独角麟', '火麒', '麒麟神', '圣光麒麟'],
    fallback: ['🥚', '🐐', '🦌', '🦄', '🔥', '👑', '✨'],
    color: '#eab308',
  },
  bg: {
