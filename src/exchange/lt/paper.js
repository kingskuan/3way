// Lighter Paper (synthetic) 适配器 — 展示为 LT: PAPER。
// Lighter 是 zkLighter L2 上的 zk perp DEX（Elliot Labs 出品），USDC 结算，
// 零手续费给零售，主流币 BTC/ETH/SOL 等。paper 提供 BTC/ETH/SOL/DOGE 4 市场的
// 合成价格（跟 bg/bu 一致的随机游走）。
// LIVE 走 lighter.js（读端点 + 写端 stub · 因需 Go 签名库 FFI，Round N+1 补齐）。
import { EventEmitter } from 'events';

const MARKETS = [
  { marketId: 1, symbol: 'BTC',  displayName: 'BTC-USDC',  basePrice: 72000, priceTick: 0.1,     sizeTick: 0.0001, minSize: 0.0001, maxLev: 20 },
  { marketId: 2, symbol: 'ETH',  displayName: 'ETH-USDC',  basePrice: 3800,  priceTick: 0.01,    sizeTick: 0.001,  minSize: 0.001,  maxLev: 20 },
  { marketId: 3, symbol: 'SOL',  displayName: 'SOL-USDC',  basePrice: 165,   priceTick: 0.001,   sizeTick: 0.01,   minSize: 0.01,   maxLev: 20 },
  { marketId: 4, symbol: 'DOGE', displayName: 'DOGE-USDC', basePrice: 0.18,  priceTick: 0.00001, sizeTick: 1,      minSize: 1,      maxLev: 20 },
];

export class LighterPaper extends EventEmitter {
  ...
  async placeLimitOrder(o) {
    ...
    const orderId = 'lt-paper-' + Date.now() + '-' + Math.random().toString(36).slice(2, 8);
  ...
  setActiveMarket(_marketId) { /* no-op in paper */ }
  ...
  async getStats() { return { volume: 0 }; }
}
