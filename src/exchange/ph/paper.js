// Phoenix Paper (synthetic) adapter — 展示为 PH: PAPER。
// Phoenix 是 Solana 链上永续 DEX（perp-api.phoenix.trade）。paper 提供
// SOL/BTC/ETH/ENA 4 市场的合成价格（跟他们主流市场对齐）。
// LIVE 走 phoenix.js —— 需要 Solana wallet keypair 签名（PH_WALLET_PRIVATE_KEY）
// 和 Solana RPC endpoint（PH_SOLANA_RPC_URL）。Round 210 完整 LIVE 实现。
import { EventEmitter } from 'events';

const MARKETS = [
  { marketId: 1, symbol: 'SOL', displayName: 'SOL-PERP', basePrice: 200,    priceTick: 0.001, sizeTick: 0.01,  minSize: 0.01,  maxLev: 25 },
  { marketId: 2, symbol: 'BTC', displayName: 'BTC-PERP', basePrice: 63000,  priceTick: 0.1,   sizeTick: 0.0001, minSize: 0.0001, maxLev: 25 },
  { marketId: 3, symbol: 'ETH', displayName: 'ETH-PERP', basePrice: 2100,   priceTick: 0.01,  sizeTick: 0.001, minSize: 0.001, maxLev: 25 },
  { marketId: 4, symbol: 'ENA', displayName: 'ENA-PERP', basePrice: 0.75,   priceTick: 0.00001, sizeTick: 1,   minSize: 1,     maxLev: 20 },
];

export class PhoenixPaper extends EventEmitter {
  ...
  async placeLimitOrder(o) {
    ...
    const orderId = 'ph-paper-' + Date.now() + '-' + Math.random().toString(36).slice(2, 8);
